...
def get_paintings(db: Session = Depends(get_db), limit: int = 10, page: int = 1, search: str = '',collection:str=''):
    ...
    skip = (page - 1) * limit
    if(collection != ""):
        paintings = db.query(models.PaintingMobile).filter(
            models.PaintingMobile.collection == collection
        ).filter(
            models.PaintingMobile.title.contains(search)).limit(limit).offset(skip).all()
    else:
          paintings = db.query(models.PaintingMobile).filter(
            models.PaintingMobile.title.contains(search)).limit(limit).offset(skip).all()
...
def create_painting(payload: schemas.PaintingBaseSchema, db: Session = Depends(get_db)):
    new_painting = models.PaintingMobile(**payload.dict())
    db.add(new_painting)
    db.commit()
...
def create_paintings(payload: list[schemas.PaintingBaseSchema], db: Session = Depends(get_db)):
    new_paintings = [models.PaintingMobile(**p.dict()) for p in payload]
...
def update_painting(paintingId: str, payload: schemas.PaintingBaseSchema, db: Session = Depends(get_db)):
    painting_query = db.query(models.PaintingMobile).filter(models.PaintingMobile.id == paintingId)
    ...
    painting_query.filter(models.PaintingMobile.id == paintingId).update(update_data,
...
def get_post(paintingId: str, db: Session = Depends(get_db)):
    painting = db.query(models.PaintingMobile).filter(models.PaintingMobile.id == paintingId).first()
    if not painting:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
