...
import pandas as pd
import re
from preprocessing.metmuseum import getImageURL
...
public_domain_paintings = public_domain_paintings[abs(public_domain_paintings["ratio"] - (9/16)) < 0.1]


for i in range(len(public_domain_paintings)):
    url = public_domain_paintings.iloc[i]["Link Resource"]
